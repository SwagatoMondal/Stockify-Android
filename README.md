# Stockify
Stock Update Notifier

# Repo links
The Backend server links are as follows -

[API Server](https://github.com/anuraggoelkkc/stockify-api)

[Recommendations Service](https://github.com/anuraggoelkkc/stock-reco)

# Screenshots
## SplashScreen
The first screen of the app.

<p align="center">
  <img width="500" height="1000" src="https://github.com/SwagatoMondal/Stockify-Android/blob/main/Screenshots/Splash.png?raw=true">
</p>

## Google SignIn
Google Sign demo for the app.

<p align="center">
  <img width="500" height="1000" src="https://github.com/SwagatoMondal/Stockify-Android/blob/main/Screenshots/Login.png?raw=true">
</p>

## Home
This screen will list all the alerts the user has subscribed to.

<p align="center">
  <img width="500" height="1000" src="https://github.com/SwagatoMondal/Stockify-Android/blob/main/Screenshots/Home.png?raw=true">
</p>

## Trending
This screen will list all the trending stocks the user can subscribed to.

<p align="center">
  <img width="500" height="1000" src="https://github.com/SwagatoMondal/Stockify-Android/blob/main/Screenshots/Trending.png?raw=true">
</p>

## Search
This screen will list all the alerts available for the user to subscribe.

<p align="center">
  <img width="500" height="1000" src="https://github.com/SwagatoMondal/Stockify-Android/blob/main/Screenshots/Search.png?raw=true">
</p>

## Stock Detail
This screen will show the details the one alert.

<p align="center">
  <img width="500" height="1000" src="https://github.com/SwagatoMondal/Stockify-Android/blob/main/Screenshots/AlertDetail.png?raw=true">
</p>

## Settings
This screen will show user level settings.

<p align="center">
  <img width="500" height="1000" src="https://github.com/SwagatoMondal/Stockify-Android/blob/main/Screenshots/Settings.png?raw=true">
</p>
